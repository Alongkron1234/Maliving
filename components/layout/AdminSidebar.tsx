...
export default function AdminSidebar() {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-64 bg-white border-r border-[#E4E4E7] flex flex-col shrink-0 print:hidden
      ...
      >
        ...
        <div className="px-5 py-5 border-b border-[#E4E4E7] flex items-center justify-between">
          <div>
            <p className="text-base font-bold text-[#C2410C] leading-tight">Maliving Admin</p>
            <p className="text-xs text-[#71717A] mt-0.5">Dormitory Management</p>
          </div>
          <button
            onClick={() => setIsOpen(false)}
            className="lg:hidden p-1 text-[#71717A] hover:text-[#3F3F46]"
            aria-label="ปิดเมนู"
          >
            <X size={18} />
          </button>
              ...
              <Link
                ...
                className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${
                  isActive
                    ...
                    : 'text-[#71717A] hover:bg-[#FFE8D1] hover:text-[#3F3F46]'
                }`}
              >
        ...
        {/* Bottom */}
        <div className="px-3 py-4 border-t border-[#E4E4E7] space-y-0.5">
          <Link
            href="/admin/help"
            onClick={() => setIsOpen(false)}
